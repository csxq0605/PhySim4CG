"""
Taichi FLIP fluid demo - Lab 2 bonus B1

Implemented features:
- baseline Lab 2 FLIP solver configuration
- particle visualization by speed / density / pressure
- mouse-draggable spherical obstacle with velocity injection
"""
...
OBSTACLE_DEFAULT_POS = np.array([0.22, 0.10, 0.02], dtype=np.float32)
OBSTACLE_MIN_POS = DOMAIN_MIN + CELL_SIZE + OBSTACLE_RADIUS
OBSTACLE_MAX_POS = DOMAIN_MAX - CELL_SIZE - OBSTACLE_RADIUS

WINDOW_RES = (1440, 960)
GUI_PANEL_BOUNDS = (0.02, 0.02, 0.26, 0.20)
CAMERA_FOV = 45.0
...
obstacle_pos = ti.Vector.field(3, dtype=ti.f32, shape=1)
obstacle_vel = ti.Vector.field(3, dtype=ti.f32, shape=1)
...
@ti.func
def type_is_solid(cell_value):
    return cell_value == SOLID_CELL or cell_value == OBSTACLE_CELL


@ti.func
def type_is_obstacle(cell_value):
    return cell_value == OBSTACLE_CELL


@ti.func
def cell_is_solid(i, j, k):
    return type_is_solid(cell_type[i, j, k])
...
def u_face_is_solid(i, j, k):
    return (
        i == 0
        or i == GRID_RES
        or type_is_solid(cell_type[i - 1, j, k])
        or type_is_solid(cell_type[i, j, k])
    )
...
def v_face_is_solid(i, j, k):
    return (
        j == 0
        or j == GRID_RES
        or type_is_solid(cell_type[i, j - 1, k])
        or type_is_solid(cell_type[i, j, k])
    )
...
def w_face_is_solid(i, j, k):
    return (
        k == 0
        or k == GRID_RES
        or type_is_solid(cell_type[i, j, k - 1])
        or type_is_solid(cell_type[i, j, k])
    )
...
def mark_obstacle_cells():
    obstacle_center = obstacle_pos[0]
    obstacle_radius = OBSTACLE_RADIUS + 0.5 * CELL_SIZE
    for i, j, k in cell_type:
        if cell_type[i, j, k] == EMPTY_CELL:
            center = ti.Vector(
                [
            ...
            if (center - obstacle_center).norm() < obstacle_radius:
                cell_type[i, j, k] = OBSTACLE_CELL
...
def mark_fluid_cells_from_particles():
    for p in particle_pos:
        if particle_active[p] != 0:
            ...
            if not cell_is_solid(idx[0], idx[1], idx[2]):
...
def apply_solid_velocity_constraints():
    obstacle_velocity = obstacle_vel[0]
    ...
    for i, j, k in grid_u:
        if i == 0 or i == GRID_RES:
            ...
            if type_is_solid(left_type) or type_is_solid(right_type):
                grid_u[i, j, k] = (
                    obstacle_velocity[0]
                    if type_is_obstacle(left_type) or type_is_obstacle(right_type)
                    else 0.0
                )
    ...
    for i, j, k in grid_v:
        if j == 0 or j == GRID_RES:
            ...
            if type_is_solid(down_type) or type_is_solid(up_type):
                grid_v[i, j, k] = (
                    obstacle_velocity[1]
                    if type_is_obstacle(down_type) or type_is_obstacle(up_type)
                    else 0.0
                )
    ...
    for i, j, k in grid_w:
        if k == 0 or k == GRID_RES:
            ...
            if type_is_solid(back_type) or type_is_solid(front_type):
                grid_w[i, j, k] = (
                    obstacle_velocity[2]
                    if type_is_obstacle(back_type) or type_is_obstacle(front_type)
                    else 0.0
                )
...
def pressure_projection_iteration(dt: ti.f32, over_relaxation: ti.f32, compensate_drift: ti.i32):
    ...
    for i, j, k in ti.ndrange(GRID_RES, GRID_RES, GRID_RES):
        if cell_is_fluid(i, j, k):
            sx0 = 0.0 if i == 0 or type_is_solid(cell_type[i - 1, j, k]) else 1.0
            sx1 = 0.0 if i == GRID_RES - 1 or type_is_solid(cell_type[i + 1, j, k]) else 1.0
            sy0 = 0.0 if j == 0 or type_is_solid(cell_type[i, j - 1, k]) else 1.0
            sy1 = 0.0 if j == GRID_RES - 1 or type_is_solid(cell_type[i, j + 1, k]) else 1.0
            sz0 = 0.0 if k == 0 or type_is_solid(cell_type[i, j, k - 1]) else 1.0
            sz1 = 0.0 if k == GRID_RES - 1 or type_is_solid(cell_type[i, j, k + 1]) else 1.0
...
@ti.kernel
def set_obstacle_state(
    center: ti.types.vector(3, ti.f32),
    velocity: ti.types.vector(3, ti.f32),
):
    obstacle_pos[0] = center
    obstacle_vel[0] = velocity
...
def initialize_scene():
    ...
    set_obstacle_state(
        ti.Vector(OBSTACLE_DEFAULT_POS.tolist()),
...
def handle_particle_collisions():
    obstacle_center = obstacle_pos[0]
    obstacle_velocity = obstacle_vel[0]
    min_dist = OBSTACLE_RADIUS + PARTICLE_RADIUS
    ...
    for p in particle_pos:
        if particle_active[p] != 0:
            ...
            offset = pos - obstacle_center
            dist = offset.norm()
            if dist < min_dist:
                normal = ti.Vector([1.0, 0.0, 0.0])
                if dist > 1e-6:
                    normal = offset / dist
                pos = obstacle_center + normal * min_dist

                relative_velocity = vel - obstacle_velocity
                normal_speed = relative_velocity.dot(normal)
                if normal_speed < 0.0:
                    relative_velocity -= normal_speed * normal
                vel = obstacle_velocity + relative_velocity
...
def clamp_obstacle_position(pos: np.ndarray) -> np.ndarray:
    return np.clip(pos, OBSTACLE_MIN_POS, OBSTACLE_MAX_POS).astype(np.float32)


def set_obstacle_state_np(center: np.ndarray, velocity: np.ndarray):
    set_obstacle_state(
        ti.Vector(center.astype(np.float32).tolist()),
        ti.Vector(velocity.astype(np.float32).tolist()),
    )
...
def screen_ray_direction(mouse_x: float, mouse_y: float) -> np.ndarray:
    forward, right, up = camera_basis()
    aspect = WINDOW_RES[0] / WINDOW_RES[1]
    tan_half_fov = np.tan(np.deg2rad(CAMERA_FOV) * 0.5)
    ndc_x = 2.0 * mouse_x - 1.0
    ndc_y = 2.0 * mouse_y - 1.0
    ray = forward + ndc_x * aspect * tan_half_fov * right + ndc_y * tan_half_fov * up
...
def cursor_over_gui(mouse_x: float, mouse_y: float) -> bool:
    x0, y0, x1, y1 = GUI_PANEL_BOUNDS
    return x0 <= mouse_x <= x1 and y0 <= mouse_y <= y1
...
def print_controls():
    print("=" * 64)
    print("Taichi Lab 2 - FLIP bonus B1")
    print("=" * 64)
    print("Controls:")
    print("  Space    : pause / resume")
    print("  R        : reset particles and obstacle")
    print("  G        : toggle tank wireframe")
    print("  Tab      : cycle base / speed / density / pressure colors")
    print("  LMB drag : move the spherical obstacle in the screen plane")
...
def draw_status_panel(
    ...
    gui.begin("Lab 2 B1 Controls", 0.02, 0.02, 0.26, 0.20)
...
def main():
    ...
    window = ti.ui.Window("Taichi FLIP Fluid - Bonus B1", res=WINDOW_RES, vsync=True)
    ...
    drag_plane_normal = np.array([0.0, 0.0, 1.0], dtype=np.float32)
    drag_offset = np.zeros(3, dtype=np.float32)
    ...
    while window.running:
        ...
        if window.is_pressed(ti.ui.LMB):
            mouse_x, mouse_y = window.get_cursor_pos()
            if not cursor_over_gui(mouse_x, mouse_y):
                ray_dir = screen_ray_direction(mouse_x, mouse_y)
                obstacle_center = obstacle_pos.to_numpy()[0]
                if not dragging_obstacle:
                    dragging_obstacle = True
                    drag_plane_point = obstacle_center.astype(np.float32)
                    drag_plane_normal = camera_basis()[0]
                    hit_point = intersect_ray_plane(
                        camera_pos,
                        ray_dir,
                        drag_plane_point,
                        drag_plane_normal,
                    ...
                    if hit_point is None:
                        ...
                        drag_offset = obstacle_center - hit_point.astype(np.float32)
                else:
                    hit_point = intersect_ray_plane(
                        camera_pos,
                        ray_dir,
                    ...
                    if hit_point is not None:
                        new_center = clamp_obstacle_position(hit_point.astype(np.float32) + drag_offset)
                        velocity = (new_center - obstacle_center) / max(frame_dt, 1e-6)
                        set_obstacle_state_np(new_center, velocity)
            else:
                dragging_obstacle = False
                set_obstacle_state_np(obstacle_pos.to_numpy()[0], np.zeros(3, dtype=np.float32))
        else:
        ...
        scene.particles(obstacle_pos, radius=OBSTACLE_RADIUS, color=(0.96, 0.42, 0.20))
